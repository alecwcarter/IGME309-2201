...
void MyCamera::CalculateView(void)
{
	...
	// Get yaw and pitch from m_v3PitchYawRoll
	float yaw = glm::radians(m_v3PitchYawRoll.y);
	float pitch = glm::radians(glm::clamp(m_v3PitchYawRoll.x, -90.0f, 90.0f));

	// Use the yaw and pitch to make a vector3
	vector3 direction;
	...
	// change the target's position using qSLERP
	m_v3Target.x = -qSLERP.x;
	m_v3Target.y = -qSLERP.y;
	m_v3Target.z = qSLERP.z;
